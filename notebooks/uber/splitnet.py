...
class SplitNet(nn.Module):
    def __init__(self, in_features=2, hidden_features=64, hidden_layers=1, out_features=1, outermost_linear=True):
        super().__init__()
        self.net = []

        self.net += [SplitLayer(in_features, hidden_features)]
        self.net += [SplitLayer(hidden_features, hidden_features) for _ in range(hidden_layers)]

        if outermost_linear:
            self.net += [nn.Linear(hidden_features, out_features)]
        else:
            self.net += [SplitLayer(hidden_features, out_features)]
